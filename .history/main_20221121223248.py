...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Game(name="f"on {len(client.guilds)} servers | {config['prefix']}help"))
    #Send a message to the console to show that the bot is online
    await client.change_presence(status=discord.Activity(type=discord.ActivityType.watching, name="CHILL & FREE"))
    #Send a message to the console to show that the bot is online
    await client.change_presence(status=discord.Activity(type=discord.ActivityType.listening, name="John Week  ♌#8686"))
