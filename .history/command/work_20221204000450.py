...
class Work(commands.Cog):
    ...
    async def work(self, ctx):
        try:
            ...
            rd_sx = ['trộm ốc xe của khách đem bán:)', "được nhiều tiền trong mơ", "được khách giàu mua gói bảo dưỡng toàn phần", "đem mấy tấm nhôm, sắt thừa của xe đi bán đồng nát"]
            ...
            if message.content.lower() == '1':
                ...
                self.bot.sql.execute(f'UPDATE user_data SET user_money = user_money + {earn} WHERE user_id={ctx.author.id}')
                await ctx.send(f"Bạn cố gắng rất nhiều trong cuộc sống này {earn}$, được thì cho đi nhé <3=))")
            elif str(message.content) == "7":
                self.bot.sql.execute(f'UPDATE user_data SET user_money = user_money + {earn} WHERE user_id={ctx.author.id}')
                await ctx.send(f"Đó là đồ công quỷ bạn ăn cắp được {earn}$, tham lam là không tốt đâu nhé =))")
            elif str(message.content) == "8":
                self.bot.sql.execute(f'UPDATE user_data SET user_money = user_money + {earn} WHERE user_id={ctx.author.id}')
